...
class Encoder_ST(nn.Module):
    ...
    def forward(self, event_loc, event_time, non_pad_mask):
        """ Encode event sequences via masked self-attention. 
            Input:  event_loc: batch_size * length * dim
                    event_loc: batch_size, length
                    non_pad_mask
        """
        ...
        slf_attn_mask_keypad = get_attn_key_pad_mask(seq_k=event_one_hot, seq_q=event_one_hot) 
        slf_attn_mask_keypad = slf_attn_mask_keypad.type_as(slf_attn_mask_subseq) ##[50, 554, 554, 2]用于指示在自注意力机制中哪些键（key）位置是有效的，通常用于处理填充（padding）部分。

        slf_attn_mask = (slf_attn_mask_keypad + slf_attn_mask_subseq).gt(0)
        
        # enc_output_loc = self.event_emb_loc(event_loc) #enc_output_temporal=[bs, seq_len, d_model]
        
        slf_attn_mask = slf_attn_mask[:,:,:,0] #???

        for index in range(len(self.layer_stack)):
            enc_output_type, _ = self.layer_stack_loc[index](
                enc_output_type,
                non_pad_mask=non_pad_mask,
                slf_attn_mask=slf_attn_mask)#enc_out_loc=[50, 554, 64]

            enc_output_temporal, _ = self.layer_stack_temporal[index](
                enc_output_temporal,
                non_pad_mask=non_pad_mask,
                slf_attn_mask=slf_attn_mask)#enc_output_temporal=[50, 554, 64]

            enc_output, _ = self.layer_stack[index](
                enc_output,
                non_pad_mask=non_pad_mask,
                slf_attn_mask=slf_attn_mask)#enc_output=[50, 554, 64]
...
class Transformer_ST(nn.Module):
    ...
    def __init__(
            self, d_model=256, d_rnn=128, d_inner=1024,
            n_layers=4, n_head=4, d_k=64, d_v=64, dropout=0.1,device=None,loc_dim=1,CosSin=False):
        super().__init__()
