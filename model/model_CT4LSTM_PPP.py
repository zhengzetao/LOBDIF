...
class CT4LSTM_PPP(nn.Module):
    ...
    def forward(self, batch, mkt_state, device = torch.device('cuda')):
        ...
        for i in range(seq_length-1):
            # interfer the hawkes process at every step
            dt = seq_dt[:,i] # dt means delta_time
            cell_i, c_target_t, output, decay_i = self.lstm(x[:,i], h_t, c_t, c_target_t)
            c_t = c_target_t + (cell_i - c_target_t) * torch.exp(-(decay_i) * dt[:, None]) #eq.13
            h_t = output * torch.tanh(c_t) #eq.14
            inten_after_decay = self.intensity_layer(h_t) #eq.15
        ...
        with torch.no_grad():
            ...
            if metrics:
                ...
                for i in range(100): #100
                    ...
                    if intens_at_samples is not None: # when i > 0
